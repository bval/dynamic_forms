      ...
      def check_box_group(name, full_collection, options={}, &block)
        ...
        if block_given?
          ...
          if options.delete(:responsive)
            boxes.each do |markup, item|
              out << "#{markup}<label for='#{id_for(name, item)}' style='display:inline-block'>#{item}</label>\n"
            end
          else
            out << "<ul class='check_box_group'>"
            boxes.each do |markup, item|
              out << "<li>#{markup}<label for='#{id_for(name, item)}' style='display:inline-block'>#{item}</label></li>\n"
            end
