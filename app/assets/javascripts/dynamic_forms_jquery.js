...
function add_field(container_id, association, content) {
  ...
  $('#' + container_id).append(content.replace(regexp, new_id));
  ...
  var position_fields = $(container_id).children(".field_position");
  var max_position = Math.max.apply(Math,position_fields.map(function(n) { return n.value}));
  if(max_position == "") {
    max_position = 1;
  }
  var blank_positions = position_fields.filter(function(n) { return n.value == ""});
...
function add_field_option(link, association, content) {
  ...
  // Find the max value
  // Cycle through all the hidden position fields that are blank and update the value
  var parent_container = $(link).parents(".form_field_options");
  var position_fields = parent_container.children(".field_option_position");
  var max_position = Math.max.apply(Math,position_fields.map(function(n) { return n.value}));
  var blank_positions = position_fields.filter(function(n) { return n.value == ""});
  blank_positions.each(function(n) {
    max_position = parseInt(max_position) + 1;
...
function remove_field(link) {
  ...
  var position = $(link).parents('.field').children('.field_position').first();
  var position_value = position.value;
  position.remove();
  var parent_container = $('form_fields');
  parent_container.children(".field_position").each(function(n) {
...
function remove_field_option(link) {
  ...
  // Cycle through the the hidden position elements and if the value is greater than the value that was removed, decrement by 1
  var position = $(link).parent().prev("input[type=hidden]");
  var position_value = position.value;
  position.remove();
  var parent_container = $(link).parents(".form_field_options");
  parent_container.children(".field_option_position").each(function(n) {
    if(parseInt(n.value) > position_value) {
      var pos = parseInt(n.value) - 1;
      n.value = pos;
    }
...
function make_form_fields_sortable() {
  ...
  //#  var current_child = $('form_fields').children('div').first();
  //#  var counter = 1;
  //#  while(current_child) {
  //#    current_child.children('.field_position').first().value = counter;
...
function make_form_field_options_sortable(container_id) {
  ...
  //  var current_child = $(container_id).children('div').first();
  //  var counter = 1;
  //  while(current_child) {
  //    current_child.children('.field_option_position').first().value = counter;
  //    current_child = current_child.next('div');
  //    counter += 1;
  //  };
  //}});
